...
internal static class PredicateBuilder
{
    ...
    )
    {
        ...
        if (!HasProperty(parameter.Type, properties[0]))
            throw new ArgumentException($"{properties[0]} does not exist on type {parameter.Type.Name}.");
        ...
        foreach (var property in properties.Skip(1))
        {
            if (!HasProperty(propertyAccess.Type, property))
                throw new ArgumentException($"{property} does not exist on type {propertyAccess.Type.Name}.");
    ...
    internal static (Expression, Type) CreateKeySelector<T>(
        string path
    )
    {
        var parameter = Expression.Parameter(typeof(T), "x");
