...
public static class QueryableExtensions
{
	...
	{
		source =
				config is IFiltered filtered && IsFiltering(filtered)
						? source.ToFiltered(filtered)
						: source;
		...
		source = source.Skip((page - 1) * perPage).Take(perPage);
	...
	)
	{
		...
		return new PagedResult<T>(totalRecords, page, config.PerPage ?? 20, items);
	}

	private static IQueryable<T> ToFiltered<T>(this IQueryable<T> source, IFiltered filter) =>
	...
	private static IQueryable<T> ToSorted<T>(this IQueryable<T> source, ISorted filter)
	{
		...
		var orderByMethod = filter.IsAscending is true
				? Expression.Call(
						typeof(Queryable),
						"OrderBy",
	...
	private static bool IsSorting(ISorted sort) => !string.IsNullOrEmpty(sort.SortedProperty);
}
